...
template <typename T>
void GG<T>::simulateCarMovementFromSourceToDest()
{
    // Setting color control variable
    HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
    ...
    // setting the source distance to 0
    dist[0] = 0;
    pq.enqueue(adj_list.searchNode(0)->data);
    ...
    // printing the path on the grid
    cur = this->vertices - 1;
    while (cur != -1)
    {
        int row = cur / this->grid.cols;
        int col = cur % this->grid.cols;
        grid.indexes[row][col] = 'C';
        cur = parent[cur];
    }
